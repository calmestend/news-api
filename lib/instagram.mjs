...
const noticias = async (req, res) => {
	try {
		...
		const extractedData = json.data.user.edge_owner_to_timeline_media.edges.map(edge => {
			const { id, dimensions, display_url, edge_media_to_caption, owner, thumbnail_src, is_video, dash_info } = edge.node;
			...
			return {
				...
				thumbnail_src,
				is_video,
				video_url: is_video ? dash_info?.video_url || null : null,
			};
		});
